...
mlir::Value reorderMemoryLayoutForReading(mlir::Location &loc,
                                          mlir::OpBuilder &builder,
                                          ReorderDesc &desc,
                                          mlir::Value srcMem) {
  // Reorder memory.
  mlir::Value newMem = createReorder(loc, builder, desc, srcMem);
  // Replace uses in read operation with reordered memory.
  srcMem.replaceUsesWithIf(newMem, [&](mlir::OpOperand &operand) {
    mlir::Operation *owner = operand.getOwner();
    bool isLoad = mlir::isa<PxaLoadOp, PxaVectorLoadOp>(owner);
    bool insideReorder = newMem.getDefiningOp() == owner->getParentOp();
    return isLoad && !insideReorder;
  });
  // Update read operations using LayoutConverter.
  LayoutConverter ltConverter(desc, builder.getContext());
  ltConverter.convert(newMem);
  return newMem;
}
...
mlir::VectorType LayoutConverter::transformVectorType(mlir::VectorType type) {
  return type;
  // return mlir::VectorType::get(reorderDesc.reorderedVector,
  // type.getElementType());
}
